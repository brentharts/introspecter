"""
introspecter8.py — engine (v8): self-research — outward corpus, fetch, and model proposals.

Extends v7 (oracle) with allowlisted literature search, PDF download into corpus/,
text ingest, and architecture proposals recorded as data. The inquiry reads outward
without claiming to understand; proposals stand beside questions, not as answers.
"""
from __future__ import annotations
import ast, base64, glob, json, os, re, subprocess, sys
import numpy as np

PARENT_DOI = "local:self_11"   # self_11.pdf (built locally)
...
RESEARCH = {}   # search/fetch/ingest cache for one evolve run


def act_search(nodes, p):
    """Outward arXiv search from node words — bibliographic data, not understanding."""
    import research as R
    terms = [w for _, w, pr in nodes if w] + [pr.split()[0] for _, _, pr in nodes if pr]
    out = R.search_literature(terms, per_query=3)
    RESEARCH["search"] = out
    return {"search": {"queries": out["queries"], "n_hits": len(out["hits"]),
                       "hits": out["hits"][:6]}, "is": out["is"]}


def act_fetch(nodes, p):
    """Download allowlisted PDFs (arXiv + author viXra seeds) into corpus/."""
    import research as R
    hits = RESEARCH.get("search", {}).get("hits")
    if not hits:
        hits = R.search_literature([w for _, w, _ in nodes])["hits"]
    out = R.fetch_corpus(hits, include_vixra=True)
    RESEARCH["fetch"] = out
    return out


def act_ingest(nodes, p):
    """Extract corpus snippets overlapping authored terms — text as data."""
    import research as R
    terms = [w for _, w, _ in nodes]
    for s, (w, pr) in p.get("nodes", {}).items():
        if s in [n[0] for n in nodes]:
            terms.extend(w.split())
    out = R.ingest_corpus(terms)
    RESEARCH["ingest"] = out
    return out


def act_propose(nodes, p):
    """Propose module/circuit architectures from geometry + corpus — proposals only."""
    import research as R
    mem = _memberships(p)
    law_self = round(_feval(p["laws"][2][1], mem), 3) if len(p.get("laws", [])) > 2 else None
    terms, M = _cooccurrence(p)
    d = M.sum(axis=1)
    hub = terms[int(d.argmax())] if len(terms) else "Sigma"
    analysis = {"hub": hub, "law_self": law_self, "n_nodes": len(p["nodes"])}
    ingest = RESEARCH.get("ingest") or R.ingest_corpus([w for _, w, _ in nodes])
    out = R.propose_models(analysis, ingest)
    RESEARCH["propose"] = out
    return out


ACTIONS.update({"search": act_search, "fetch": act_fetch,
                "ingest": act_ingest, "propose": act_propose})
...
TITLE_PRETTY = {
    ...
    "act_search": "Searching Outward", "act_fetch": "Fetching the Corpus",
    "act_ingest": "Ingesting the Corpus", "act_propose": "Proposing Models",
...
def abstract(p, engine=None):
    ...
    rtext = (" It may search outward (arXiv), fetch allowlisted PDFs into a local "
             "corpus, ingest excerpts, and propose module architectures --- each "
             "operation returns bibliographic or textual data and authored proposals, "
             "not comprehension of a self." if any(a in {"search", "fetch", "ingest", "propose"}
             for _, _, a in p.get("hyper", []) if a) else "")
    return (f"Stage {n} of a self-evolving hypergraph, extended from its parent "
            ...
            f"suggestive Greek glyph with a precise description.{mtext}{ltext}{ctext}{ftext}{itext}{otext}{rtext} It "
...
for f in glob.glob("*.pdf"):
    ...
    if "introspecter8.py" in a and (cand is None or os.path.getmtime(f) > os.path.getmtime(cand)):
        cand = f
...
import importlib, introspecter8 as I; importlib.reload(I); I.evolve()
'''
def _bootstrap():
...
def build_pdf(src=None, results=None, lineage=None):
    ...
    w.add_attachment("introspecter8.py", open(__file__, "rb").read())
    rp = os.path.join(os.path.dirname(os.path.abspath(__file__)), "research.py")
    if os.path.isfile(rp):
        w.add_attachment("research.py", open(rp, "rb").read())
    ...
    for extra in ("manifold.json", "manifold.png", "research.json"):
        ...
        with zipfile.ZipFile(buf, "w") as zf:
            for fn in os.listdir(corpus):
                if fn.lower().endswith(".pdf"):
                    zf.write(os.path.join(corpus, fn), f"corpus/{fn}")
        if buf.tell():
            w.add_attachment("corpus.zip", buf.getvalue())
...
def evolve_auto(oracle=None, oracle_path=None, approve_all=True,
    ...
    global ORACLE_LOG, _LAST_ORACLE_TEXT, RESEARCH
    RESEARCH.clear()
