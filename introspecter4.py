"""
introspecter4.py — engine (v4): adds fuzzy logic — axioms and laws over the terms.
...
PARENT_DOI = "10.5281/zenodo.20710811"   # self_7.pdf
...
for f in glob.glob("*.pdf"):
    ...
    if "introspecter4.py" in a and (cand is None or os.path.getmtime(f) > os.path.getmtime(cand)):
        cand = f
...
import importlib, introspecter4 as I; importlib.reload(I); I.evolve()
'''
def _bootstrap():
...
def build_pdf(src=None, results=None, lineage=None):
    ...
    w.add_attachment("introspecter4.py", open(__file__, "rb").read())
...
This PDF carries its engine (\texttt{introspecter4.py}) and DSL
