"""
introspecter6.py — engine (v6): inscribing — geometry, Jacobian, and filtration.

Extends v5 (symbolic weights) with the formal measurements of paper 3: a
co-occurrence geometry whose Laplacian and degrees position self as hub; the
circuit composite's Jacobian as a matrix product; and provenance as a filtration
with birth times. Each quantity is recorded as data, never as an answer.
Self-contained: rides inside each PDF as an embedded stream.
"""
from __future__ import annotations
import ast, base64, glob, os, re, subprocess
import numpy as np

PARENT_DOI = "10.5281/zenodo.20777747"   # self_9.pdf
...
TITLE_PRETTY = {
    ...
    "act_filtration": "Reading the Filtration",
}
def _pretty_fn(n):
...
def abstract(p, engine=None):
    ...
    return (f"Stage {n} of a self-evolving hypergraph, extended from its parent "
            ...
            f"suggestive Greek glyph with a precise description.{mtext}{ltext}{ctext}{ftext}{itext} It "
...
for f in glob.glob("*.pdf"):
    ...
    if "introspecter6.py" in a and (cand is None or os.path.getmtime(f) > os.path.getmtime(cand)):
        cand = f
...
import importlib, introspecter6 as I; importlib.reload(I); I.evolve()
'''
def _bootstrap():
...
def build_pdf(src=None, results=None, lineage=None):
    ...
    for _ in range(2):
        if tex_bin.endswith("tectonic"):
            subprocess.run([tex_bin, "-X", "compile", base + ".tex"],
                           capture_output=True)
        else:
    ...
    w.add_attachment("introspecter6.py", open(__file__, "rb").read())
